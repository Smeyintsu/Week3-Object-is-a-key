...
function cariModus(arr) {
    let highFreq = {};
    for (let num of arr) {
        highFreq[num] = (highFreq[num] || 0) + 1;
    }

    let findModus = 0;
    for (const key in highFreq) {
        if (findModus < highFreq[key]) {
            findModus = highFreq[key];
        }
    }

    if (findModus === arr.length || findModus === 1) {
        return -1;
    }

    for (let num of arr) {
        if (findModus === highFreq[num]) {
            return num;
        }
    }
  }
